...
#生成ros dns v7.6之后
${sudoCmd} sed 's/\(.*\)/add name="\1" type=FWD forward-to=$netflix comment=Netflix/g' netflix_d.txt > netflix.txt
${sudoCmd} sed 's/\(.*\)/add name="\1" type=FWD forward-to=$netflix match-subdomain=yes comment=Netflix/g' netflix_ds.txt >> netflix.txt
${sudoCmd} sed 's/\(.*\)/add regexp="\1" type=FWD forward-to=$netflix comment=Netflix/g' netflix_dk.txt >> netflix.txt
${sudoCmd} sed '=' netflix.txt | sed -r 'N;s/([^\n]+)\n(.*)/\2/' > netflix.dns
