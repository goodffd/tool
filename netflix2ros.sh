...
#生成ros dns v7.6之后
${sudoCmd} sed 's/\(.*\)/:do { add name="\1" type=FWD forward-to=$netflix match-subdomain=yes address-list=netflix comment=Netflix } on-error={}/g' netflix_d.txt > netflix.dns > netflix.dnsv6
${sudoCmd} sed 's/\(.*\)/:do { add name="\1" type=FWD forward-to=$netflix match-subdomain=yes address-list=netflix comment=Netflix } on-error={}/g' netflix_ds.txt >> netflix.dns >> netflix.dnsv6
${sudoCmd} sed 's/\(.*\)/:do { add regexp="\1" type=FWD forward-to=$netflix address-list=netflix comment=Netflix } on-error={}/g' netflix_dk.txt >> netflix.dns >> netflix.dnsv6
...
${sudoCmd} sed -i '2 i/ip dns static remove [/ip dns static find comment="Netflix"]' netflix.dnsv6
