...
unlockdns=$(dig tw6.dnsunlock.com +short)
...
sed -i '1 i/ip firewall layer7-protocol set [find name=disney] regexp=' Disney.list.rosL7

#生成ros dns
sed -i 's/\./\\\\./g;s/\(.*\)/add regexp="(\\\\.|^)\1\\$" type=A address=$disney comment=DN/g' Disney.list
sed '=' Disney.list | sed -r 'N;s/([^\n]+)\n(.*)/\2\1/' > Disney.list.rosdns
sed -i "1 i:local disney $unlockdns" Disney.list.rosdns
